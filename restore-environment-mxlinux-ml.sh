...
LOG_FILE="$RESULTS/restore-mxlinux-ml-$(date +%Y%m%d-%H%M%S).log"
exec > >(tee -a "$LOG_FILE") 2>&1

log() { printf '[restore-mxlinux-ml] %s\n' "$*" >&2; }
skip() { log "Skip: $*"; }

command -v apt-get &>/dev/null || { log "apt-get not found. This script is for MX Linux (apt-based) systems."; exit 1; }
