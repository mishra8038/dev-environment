...
LOG_FILE="$RESULTS/restore-endeavour-ml-$(date +%Y%m%d-%H%M%S).log"
exec > >(tee -a "$LOG_FILE") 2>&1

log() { printf '[restore-endeavour-ml] %s\n' "$*" >&2; }
skip() { log "Skip: $*"; }
